...
class Encoder(nn.Module):
    def __init__(self, obs_shape, repr_dim, proj_dim):
        super().__init__()
        ...
        self.repr_dim = repr_dim  # 32 * 35 * 35

        self.intm_projector = nn.Linear(39200, repr_dim)
        self.act = nn.ReLU()
    ...
    def encode(self, obs):
        obs = obs / 255.
        h = self.conv(obs)
        h = self.intm_projector(h.view(h.shape[0], -1))
...
class TransitionDynamics(nn.Module):
    def __init__(self, input_shape, action_shape, hidden_shapes, output_shape, use_rnn=False):
        super().__init__()
        hidden_shapes = OmegaConf.to_container(hidden_shapes)
        action_shape = OmegaConf.to_container(action_shape)
        self.use_rnn = use_rnn
        if use_rnn:
            self.rnn_cell = nn.GRU(input_shape + action_shape[0], output_shape, batch_first=True)     
            self.hidden0 = torch.zeros([output_shape])
        else:
            shapes = [input_shape+action_shape[0]] + hidden_shapes + [output_shape]
            layer_list = [[nn.Linear(shapes[i], shapes[i+1]), nn.ReLU()] for i in range(len(shapes)-2)] + [[nn.Linear(shapes[-2], shapes[-1])]]
            self.model = nn.Sequential(*chain(*layer_list))            
        self.apply(utils.weight_init)

    def forward(self, obs, action, time):
        x = torch.cat([obs, action], axis=-1)        
        if self.use_rnn:    
            if time == 0:
                batch_size = obs.size(0)
                self.hidden = self.hidden0.to(obs.device).tile([1, batch_size, 1])
            output, self.hidden = self.rnn_cell(x[:,None], self.hidden)
            pred_next_obs = output[:, 0]            
        else:
            pred_next_obs = self.model(x)
...
class ProtoAgent(object):
    def __init__(self, obs_shape, action_shape, action_range, device,
                 encoder_cfg, trans_dyn_cfg, critic_cfg, actor_cfg, proto_cfg,
                 ...
                 task_agnostic, intr_coef, trans_coef, num_seed_steps):
        ...
        self.trans_dyn = hydra.utils.instantiate(trans_dyn_cfg).to(self.device)
    ...
    def update_repr(self, obs, next_obs, obs_h0, action_h, next_obs_h):
        # transition dyn.
        lat0 = self.encoder.encode(obs_h0)
        with torch.no_grad():
            target_lat = self.encoder_target.encode(next_obs_h.view(-1, *next_obs_h.shape[2:]))
            target_lat = target_lat.view(*next_obs_h.shape[:2], -1)
            # target_z = self.encoder_target.project(target_lat)
        lat = lat0
        lats = []
        loss_trans = 0        
        # z0 = self.encoder.project(lat0)                
        # loss_protos = []
        for i in range(action_h.size(1)):  # horizon length
            lat = self.trans_dyn(lat, action_h[:, i], i)
            lats.append(lat)
            # loss_protos.append(self.proto(z0, target_z[:, i]))
        # loss_protos = torch.stack(loss_protos).sum()            
        loss_trans = self.compute_trans_loss(lats, target_lat)
        ...
        loss_proto = self.proto(z, next_z)

        # optimize for total losses 
        # loss_repr = loss_protos + self.trans_coef * loss_trans
        loss_repr = loss_proto + loss_trans
        self.proto_optimizer.zero_grad()
        loss_repr.backward()
        self.proto_optimizer.step()

        return loss_proto, loss_trans  # for debug

    def compute_trans_loss(self, input: list, target: torch.Tensor):
        '''
        cosine similarity loss introduced in SPR:  
        https://github.com/mila-iqia/spr/blob/release/src/models.py
        '''
        input_torch = torch.stack(input, axis=1)
        input_normed = F.normalize(input_torch.float(), p=2., dim=-1, eps=1e-3)        
        target_normed = F.normalize(target.float(), p=2., dim=-1, eps=1e-3)        
        loss = F.mse_loss(input_normed, target_normed, reduction="none").sum(1).mean()
        return loss
    ...
    def update(self, replay_buffer, trans_buffer, step):
        ...
        if self.task_agnostic:
            if step % self.encoder_update_frequency == 0:
                obs_h0, action_h, next_obs_h = trans_buffer.sample(self.batch_size)
                losses = self.update_repr(obs, next_obs, obs_h0, action_h, next_obs_h)
